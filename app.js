...
app.post('/admin/removecourses', (req, res) => {
    console.log('log1')
    let courseid = req.body.course;
    console.log(courseid)
    let sql = `DELETE FROM studentcourse WHERE Courses_courseid='${courseid}'`
    db.query(sql, (err, result) => {
        if (err) {
            return res.status('500').send('Server Error')
        }
        let sql = `DELETE FROM courses WHERE courseid='${courseid}'`
        db.query(sql, (err, result) => {
            console.log('log2')
            if (err) {
                console.log(err)
